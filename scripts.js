...
// Sample comments for comments.json
const sampleComments = [
    {
        email: "john@example.com",
        color: "blue",
        comment: "Great hostel with amazing views!",
        upvotes: 5,
        downvotes: 1
    },
    {
        ...
        color: "green",
        comment: "The community here is wonderful!",
        upvotes: 3,
        downvotes: 0
    },
    {
        ...
        color: "red",
        comment: "Room 111 might be small but the discount is worth it!",
        upvotes: 2,
        downvotes: 1
    }
];
...
commentForm.addEventListener('submit', (e) => {
    ...
    const commentDiv = createCommentElement(commentData);
    commentsList.insertBefore(commentDiv, commentsList.firstChild);

    saveComments([...commentsList.children].map(el => el.data));
    commentForm.reset();
});

function saveComments(comments) {
    fetch("http://127.0.0.1:8080/comments.json", {
        method: "PUT",
        headers: {
    ...
    fetch("http://127.0.0.1:8080/comments.json")
        ...
        .then(data => {
            ...
            if (sortSelect.value === 'popular') {
                ...
                // Latest first (assuming the array is in chronological order)
                data.reverse();
            }
        ...
        .catch(error => {
            ...
            // Load sample comments if fetch fails
...
function createCommentElement({ email, color, comment, upvotes = 0, downvotes = 0 }) {
    const commentDiv = document.createElement('div');
    commentDiv.classList.add('card', `comment-${color}`);
    commentDiv.data = { email, color, comment, upvotes, downvotes };
    ...
    commentDiv.innerHTML = `
        <div class="card-body">
            ...
            <div class="vote-buttons">
                <button class="btn btn-sm btn-success upvote">👍</button>
                <span class="vote-count">${upvotes - downvotes}</span>
                <button class="btn btn-sm btn-danger downvote">👎</button>
            </div>
        </div>
    `;

    // Add voting functionality
    const upvoteBtn = commentDiv.querySelector('.upvote');
    const downvoteBtn = commentDiv.querySelector('.downvote');
    const voteCount = commentDiv.querySelector('.vote-count');

    upvoteBtn.addEventListener('click', () => {
        commentDiv.data.upvotes++;
        voteCount.textContent = commentDiv.data.upvotes - commentDiv.data.downvotes;
        saveComments([...commentsList.children].map(el => el.data));
